...
def create_app():
    ...
    def book(competition, club):
        ...
        if (
            datetime(
                int(date[0:4]), int(date[5:7]), int(date[8:10]), int(date[11:13]), int(date[14:16]), int(date[17:19])
            )
            < datetime.now()
        ):
    ...
    def purchasePlaces():
        ...
        clubPlaceBooked = getattr(club["booked"], competition["name"], 0)
        if (clubPlaceBooked + placesRequired > 12) or placesRequired > 12:
            return render_template(
                "welcome.html",
                club=club,
                competitions=competitions,
                errors=["You can't book more than 12 places in 1 competition"],
            )
        else:
